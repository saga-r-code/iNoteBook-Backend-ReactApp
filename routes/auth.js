...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user) {
        //if already exist than return otherwise create new user
        return res
          .status(400)
      ...
      const data = {
        //  the payload for the JWT with additional claims and data is encode throgh "jwt"
        user: {
          id: user.id,
        },
      };
      ...
      res.send({ response: req.body, success });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({ error: "please try to fill correct information", success }); //user not found then error show
      } else {
      ...
      if (!comparePassword) {
        return res
          .status(400)
          .json({ error: "please try to fill correct information", success });
      } else {
